<template>
 ...
 <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('asaar')">Asars</inertia-link>
      <span class="text-indigo-400 font-medium">/</span>
      {{ form.ownership }} by {{ form.ownership_name }} xd {{asar.id}}
    </h1>
    <trashed-message v-if="asar.deleted_at!=null && asar.deleted_at!='no'" class="mb-6" @restore="restore">
      This asar has been deleted.
    </trashed-message>
    <div class="bg-white rounded shadow overflow-hidden md:max-w-xlg">
      <!-- <form @submit.prevent="submit">
        ...
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          ...
          <select-input v-model="form.asaar_id" :error="errors.asaar_id" class="pr-6 pb-8 w-full lg:w-1/2" label="Asar">
            <option :value="null" />
            <option v-for="asaar in asaars" :key="asaar.id" :value="asaar.id">{{ asaar.name }}</option>
          </select-input>
        ...
        <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex items-center">
          <button v-if="!asar.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Delete Asar</button>
          <loading-button :loading="sending" class="btn-indigo ml-auto" type="submit">Update Asar</loading-button>
        </div>
      </form> -->
       ...
       <form @submit.prevent="submit">
        ...
        ></b-form-datepicker> -->
          ...
          <!-- <select-input
            v-model="form.asaar_id"
            :error="errors.asaar_id"
            class="pr-6 pb-8 w-full lg:w-1/2"
            label="Asar"
          >
            <option :value="null" />
            <option
              v-for="asaar in asaars"
              :key="asaar.id"
              :value="asaar.id"
            >
              {{ asaar.name }}
         ...
         <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex items-center">
          <button v-if="asar.deleted_at!=null && asar.deleted_at=='no'" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Delete Asar</button>
          <loading-button :loading="sending" class="btn-indigo ml-auto" type="submit">Update Asar</loading-button>
        </div>
      </form>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
       id: this.asar.id,
                image: this.asar.image,
                ownership: this.asar.ownership,
                ownership_name: this.asar.ownership_name,
                image_id: this.asar.image_id,
                nadim_image_id: this.asar.nadim_image_id,
                image_type_slide: this.asar.image_type_slide,
                image_type_slide_size: this.asar.image_type_slide_size,
                image_type_slide_frame: this.asar.image_type_slide_frame,
                image_type_negative: this.asar.image_type_negative,
                image_type_negative_size: this.asar.image_type_negative_size,
                image_type_negative_material: this.asar.image_type_negative_material,
                image_type_print: this.asar.image_type_print,
                image_type_print_size: this.asar.image_type_print_size,
                image_type_print_type: this.asar.image_type_print_type,
                image_type_print_side: this.asar.image_type_print_side,
                image_type_print_side_recto_sig: this.asar.image_type_print_side_recto_sig,
                image_type_print_side_recto_sig_type: this.asar.image_type_print_side_recto_sig_type,
                image_type_print_side_recto_sig_name: this.asar.image_type_print_side_recto_sig_name,
                image_type_print_side_recto_other_marks: this.asar.image_type_print_side_recto_other_marks,
                image_type_print_side_verso_printed_info: this.asar.image_type_print_side_verso_printed_info,
                image_type_print_side_verso_hand_written: this.asar.image_type_print_side_verso_hand_written,
                image_type_print_side_verso_lang: this.asar.image_type_print_side_verso_lang,
                image_type_print_side_verso_dec_text: this.asar.image_type_print_side_verso_dec_text,
                image_type_print_printed_type: this.asar.image_type_print_printed_type,
                image_type_print_printed_material: this.asar.image_type_print_printed_material,
                image_type_print_printed_mount: this.asar.image_type_print_printed_mount,
                image_type_print_printed_orientation: this.asar.image_type_print_printed_orientation,
                photografic_process: this.asar.photografic_process,
                photografic_process_digital_metadata: this.asar.photografic_process_digital_metadata,
                description: this.asar.description,
                image_geographic_location_country: this.asar.image_geographic_location_country,
                image_geographic_location_gov: this.asar.image_geographic_location_gov,
                image_geographic_location_city: this.asar.image_geographic_location_city,
                image_geographic_location_city_specific_site: this.asar.image_geographic_location_city_specific_site,
                image_geographic_location_city_address: this.asar.image_geographic_location_city_address,
                image_geographic_location_city_desc_of_site: this.asar.image_geographic_location_city_desc_of_site,
                photographer_studio: this.asar.photographer_studio,
                event_date :  this.asar.event_date,
                event_date_exact_date_btw: this.asar.event_date_exact_date_btw,
                event_date_exact_date_century: this.asar.event_date_exact_date_century,
                image_scanning_date: this.asar.image_scanning_date,
                image_subject_monument: this.asar.image_subject_monument,
                image_subject_area_in_monument: this.asar.image_subject_area_in_monument,
                image_subject_item_artifact_in_monument: this.asar.image_subject_item_artifact_in_monument,
                image_subject_event: this.asar.image_subject_event,
                image_subject_event_detail: this.asar.image_subject_event_detail,
                image_subject_activity: this.asar.image_subject_activity,
                image_subject_object: this.asar.image_subject_object,
                image_subject_people: this.asar.image_subject_people,
                image_subject_people_detail: this.asar.image_subject_people_detail,
                image_subject_other: this.asar.image_subject_other,
                type_of_camera: this.asar.type_of_camera,
                keywords: this.asar.keywords,
                image_status_condition: this.asar.image_status_condition,
                image_status_needs_repair: this.asar.image_status_needs_repair,
                comment: this.asar.comment,
                digital_location_of_image_file: this.asar.digital_location_of_image_file,
                file_size: this.asar.file_size,
                copywrite: this.asar.copywrite,
                bibliography_citation: this.asar.bibliography_citation,
                icons_for_sharing_on_social_media: this.asar.icons_for_sharing_on_social_media,
                nadim_foundation_copy_right_statement: this.asar.nadim_foundation_copy_right_statement,
                deleted_at: this.asar.deleted_at
  ...
  methods: {
    submit() {
      this.$inertia.put(this.route('asaar.update', this.asar.id), this.form, {
        onStart: () => this.sending = true,
        onFinish: () => this.sending = false,
      })
    },
    destroy() {
      if (confirm('Are you sure you want to delete this asaar?')) {
        this.$inertia.delete(this.route('asaar.destroy', this.asar.id))
      }
    },
    restore() {
      if (confirm('Are you sure you want to restore this asaar?')) {
        this.$inertia.put(this.route('asaar.restore', this.asar.id))
      }
    },
    ...
    getImage(obj) {
      ...
      console.log(obj);
      console.log(obj.original.base64);
      this.form.image = obj.original.base64;
      console.log("xdxd");
    },
  },
}
